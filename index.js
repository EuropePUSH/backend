// index.js (full version, 2025-11-12)
// ----------------------------------
...
const TT_REDIRECT_URL = process.env.TIKTOK_REDIRECT_URL || `${PUBLIC_BASE_URL}/auth/tiktok/callback`;
...
app.get("/auth/tiktok/callback", async (req, res) => {
  ...
  try {
    ...
    res.type("html").send(`
      ...
      <p><b>Husk:</b> Gem access/refresh tokens i din DB.</p>
    `);
  } catch (e) {
...
app.post("/jobs", async (req, res) => {
  ...
  try {
    ...
    const inBuf = await (await fetch(source_video_url)).arrayBuffer();
    await fs.writeFile(tmpIn, Buffer.from(inBuf));
    console.log("Downloaded to file bytes:", inBuf.byteLength);
    ...
    const ffArgs = [
      ...
      "-i", tmpIn,
      "-vf", "scale=w=1080:h=1920:force_original_aspect_ratio=decrease:flags=fast_bilinear," +
